...
up_count = 0
down_count = 0
optimal_bunch_count = 0
damaged_bunch_count = 0
tracker1 = []
tracker2 = []
...
def detect(opt):
    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                if len(outputs) > 0:
                    for j, (output, conf) in enumerate(zip(outputs, confs)):
                        ...
                        # possible to call classification model here to higher accuracy
            ...
            if show_vid:
                # right vertical line
                cv2.line(im0, (w - 300, 0), (w - 300, h), (255, 0, 0), thickness=3)

                thickness = 3  # font thickness
                font = cv2.FONT_HERSHEY_PLAIN
                fontScale = 1
                cv2.putText(im0, "Optimlal bunch:  " + str(optimal_bunch_count), (10, h - 80), font,
                            fontScale, (0, 200, 0), thickness, cv2.LINE_AA)

                cv2.putText(im0, "Damaged bunch:  " + str(damaged_bunch_count), (10, h - 50), font,
                            fontScale, (255, 0, 0), thickness, cv2.LINE_AA)

                end_time = time.time()
                fps = 1 / (end_time - start_time)
                cv2.putText(im0, "FPS: " + str(int(fps)), (10, h - 10), cv2.FONT_HERSHEY_PLAIN, 1, (255, 255, 255), 2)

                im0 = cv2.resize(im0, (960, 540))
                try:
                    cv2.imshow('iCOunting grape bunchs', im0)
...
def count_obj(box, w, h, id, cls):
    global up_count, down_count, tracker1, tracker2, optimal_bunch_count, damaged_bunch_count
    ...
    if cx > (w - 300):
        if id not in tracker1:
            ...
            if cls == 0:
                optimal_bunch_count += 1
            elif cls == 1:
                damaged_bunch_count += 1
